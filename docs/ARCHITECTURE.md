...
- Owner reads use `get_owner_team`, a minimal fixed-search-path DTO that may join Auth email only after verifying the current active owner. Browser code never receives an admin client or service credential.
- `organization_invitations` is an append-preserving lifecycle record with one pending row per organization/normalized email. Supabase Auth owns and hashes the one-time email credential; ServiceFlow stores no plaintext application invitation token. Local delivery uses the existing Supabase Auth/Mailpit path.
- Creation, resend, revoke, intended-email acceptance, profile edits, and activation changes are narrow audited transactions deriving actor and tenant permission from `auth.uid()`. Accepted/revoked invitations cannot transition again; acceptance returns the existing linked membership on a safe retry.
- Member deactivation atomically marks membership/profile inactive and switches active service assignments off. This keeps Phase 4A assignment history while preventing future eligibility. Reactivation requires deliberate service reassignment. The existing deferred active-owner invariant and the managed status function independently protect the final active owner.
- `/team/accept` is authenticated but outside the owner dashboard guard because an invitee has no active tenant membership until acceptance. Every other Phase 4B route is owner-only and cross-tenant identifiers use non-disclosing unavailable/not-found behavior.

## 22. Phase 4D management-list decisions

- Management list state is parsed from asynchronous page `searchParams` in Server Components. Small shared Zod schemas normalize repeated, malformed, oversized, and out-of-range values to safe defaults; only documented filters, sorts, directions, and page sizes of 5, 10, or 20 survive. GET forms reset their own page by omitting it, while deterministic URL helpers preserve the other active state and support refresh and browser history naturally.
- `list_managed_services`, `list_owner_team_members`, `list_owner_invitations`, and `list_schedule_members` are narrow read-only security-definer projections with fixed empty `search_path`. They re-authorize `auth.uid()`, verified active membership/profile state, operational tenant state, and owner-versus-self capability before searching. Search text remains a bound SQL value; user input never selects a column or supplies tenant authority.
- Offset pagination is deliberately bounded for portfolio-scale management data. Each function returns a minimal DTO page plus exact filtered and organization totals, uses allowlisted `CASE` ordering and UUID tie-breaking, and clamps nonexistent application pages to the final valid page. Team Auth email is joined only inside the owner-authorized projection; availability staff see only their own eligible profile.
- Existing composite tenant constraints, RLS, managed-write triggers, and mutation RPCs remain unchanged. No speculative text-search index was added: the existing tenant/status/name indexes narrow these small organization lists, and trigram indexing remains measurement-driven.
## Phase 6A read model

Owner appointment screens are Server Components backed by narrow `authenticated` PostgreSQL RPCs. Each RPC calls the owner-membership guard for the server-derived active organization, applies organization-timezone date boundaries, and returns a minimal JSON projection. List queries are bounded to 5, 10, or 20 rows, use allowlisted filters and sorting, and add appointment ID as the deterministic tie-breaker. Detail projections omit event metadata, notification recipients, idempotency data, and guest-token material. Ordinary owner reads do not use a service-role client.
