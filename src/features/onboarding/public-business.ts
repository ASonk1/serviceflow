...
const publicBusinessSchema=z.object({slug:z.string(),name:z.string(),logoPath:mediaPath,city:z.string().nullable(),region:z.string().nullable(),countryCode:z.string().nullable(),timezone:z.string(),currency:z.enum(["RON","EUR","USD","GBP"]),staff:z.array(z.object({displayName:z.string(),bio:z.string().nullable(),jobTitle:z.string().nullable(),avatarPath:mediaPath})),services:z.array(z.object({name:z.string(),description:z.string().nullable(),durationMinutes:z.number(),bufferMinutes:z.number(),priceMinor:z.number()})),availability:z.array(z.object({weekday:z.number(),start:z.string(),end:z.string()}))});
