...
export default async function Page({searchParams}:{searchParams:Promise<{step?:string}>}){
 ...
 const [{data:staff},{data:service},{data:availability}]=await Promise.all([
  progress.staff_profile_id?db.from("staff_profiles").select("display_name,bio,job_title,is_public,status").eq("organization_id",organizationId).eq("id",progress.staff_profile_id).maybeSingle():Promise.resolve({data:null}),
