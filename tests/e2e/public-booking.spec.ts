...
test("public discovery and shareable availability selection",async({page})=>{
 ...
 await page.locator(".slot-button").first().click();await expect(page.getByRole("heading",{name:/Fictional Movement Session at/})).toBeVisible();await expect(page.getByText("This time is not reserved until the next step succeeds.")).toBeVisible();
 const url=page.url();await page.reload();await expect(page).toHaveURL(url);await expect(page.getByText("This time is not reserved until the next step succeeds.")).toBeVisible();
